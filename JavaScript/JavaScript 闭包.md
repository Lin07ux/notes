...
其中第三点与闭包无关，就不讨论了。文法环境中，用于解析函数执行过程中使用到的变量标识符。我们可以将文法环境想象成一个对象，该对象包含了两个组件：`环境记录(Enviroment Recode)`，和`外部引用(Out Referennce)`。环境记录包含了函数内部声明的`局部变量`和`参数变量`，外部引用指向了函数对象外部的上下文执行场景。
...
*当一个变量还在被引用，那么这个变量所在的执行环境就不会被 GC 回收。(所以闭包也很容易导致内存泄露。另外，如果两个变量是互相应用的，那还是会被回收的。)*
