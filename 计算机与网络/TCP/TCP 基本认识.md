...
类似的，如果服务端调用`listen`之后进入睡眠状态，此时也是能够完成 TCP 连接的建立的，因为连接请求的监听是由内核完成的，与服务程序无关。

#### 10.6 连接建立后，一端 sleep 休眠，write 和 send 能否成功

**可以**。

因为发送方调用`write`发送数据的时候，只要把数据从应用层拷贝到 socket 发送缓冲区，函数就返回成功了，至于什么时候发数据给对端、一次发送多少数据，就是由内核来控制的。

所以，即便一端 sleep 休眠，不再读取发过来的数据，也不会影响发送方的 write 函数的执行。同样的，即便发送方已休眠，其已发送的数据依旧会被对端 read。

![](https://cnd.qiniu.lin07ux.cn/markdown/8bf9bb5c365c83013f70233b4c66a141.png)
