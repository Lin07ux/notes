...
|  按键  |  说明                                                        |
|-------|--------------------------------------------------------------|
| i, I  | i 为目前光标所在处插入；I 为目前所在行的第一个非空格符处开始插入（常用）。
| a, A  | a 为目前光标所在的下一个字符处开始插入；A 为从光标所在行的最后一个字符处开始插入（常用）。|
| o, O  | o 为目前光标所在的下一行处插入新的一行；O 为目前光标所在哈你给的上一行插入新的一行（常用）。|
| r, R  | 进入替换模式：r 只会替换光标所在的那一个字符一次；R 会一直替换光标所在的文字，直到按下 [Esc] 键为止（常用）。|

一般模式切换到命令行模式，可用的按键如下：
...
| :n1, n2 w[filename] | 将 n1 到 n2 行的内容保存成 [filename] 这个文件。其中 n1、n2 为 Int 类型。|
