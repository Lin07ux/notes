...
代码的关键处在于`array_reduce`部分。通过这个函数，将定义的中间件串联起来。由于中间件数组`$pipe`中最前面的中间件经过`array_reduce`处理后会被放到最里面，所以在`then()`函数中，还调用了一次`array_reverse()`函数来保证最终的执行顺序和定义中间件数组时的顺序一致。
