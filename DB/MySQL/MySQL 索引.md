...
* **InnoDB 主键是聚集索引**：InnoDB 的数据文件本身就是索引文件，是按 B+Tree 组织的一个索引结构，这棵树的叶节点的 data 域保存了完整的数据记录。这棵 B+ 树的索引的 key 是数据表的主键，而辅助索引的 data 域存储相应记录主键的值而不是地址。换句话说，InnoDB 的所有辅助索引都引用主键作为 data 域。

* **MyISAM 为非聚集索引**，其 B+ 树的叶结点只是存储了数据的地址。MyISAM 引擎使用 B+Tree 作为索引结构，叶节点的 data 域存放的是数据记录的地址。在 MyISAM 中，主索引和辅助索引在结构上没有任何区别，只是主索引要求 key 是唯一的，而辅助索引的 key 可以重复。

![](http://cnd.qiniu.lin07ux.cn/markdown/1559564341860.png)
...
* **索引覆盖**：使用索引查找时，如果所需要获取的值只有索引值，不包含其他记录值，那么就可以直接返回找到的索引的值，不存在回表现象，这就被称为索引覆盖。索引覆盖对普通索引，特别是联合索引查询中非常有用，可以大大提升查询效率。
...
> 计算某列的选择性：`SELECT count(DISTINCT(title)) / count(*) AS Selectivity FROM titles;`。

### 1.2 开销和成本

索引是昂贵的，不要添加多余的索引。**在多数情况下，扩展索引比添加一个新的索引要好**。索引的开销主要有如下两个方面：
...
* 长主键索引(使所有相应的二级索引变得更长、更慢)，越长的索引通常越慢
* 随机主键索引(插入时会导致大量的页面分割)
* 低区分度的索引是低劣的
...
2. 如果表中有多个非空唯一索引，InnoDB 将选择建表时定义的第一个非空唯一索引为主键。注意：这里选择是根据定义非空唯一索引的顺序，而不是建表时对应列的顺序。
3. 不符合上述条件，InnoDB 存储引擎自动创建一个 6 个字节大小的指针，用户不能查看或访问。
