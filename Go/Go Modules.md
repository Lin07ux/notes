...
在 Go Modules 方案出来之前，社区中已经涌现了大量的依赖解决方案，包括很多人使用的 vendor 目录模式方案以及 dep 依赖管理工具。
...
在这个项目的根目录有一个 go.mod 文件，在`module/tour`目录下也有一个 go.mod 文件。为这个项目分别打两个 tag：`v0.0.1`和`module/tour/v0.0.1`。此时其模块导入和版本信息的对应关系如下：
...
文件中每一行都以一个动词开头，目前有如下几个动词：

* `module` 用于定义当前项目的模块路径，采用仓库+module name 的方式定义。因为 Go Modules 遵循语义化版本规范，所以如果工程的版本已经大于 2.0.0 了，那么按照规范就需要加上 major 版本的后缀。如下：
...
* `go` 用于设置预期的 Go 版本，但这一行并不是必须的，可以不写，目前也仅作为标识作用。

* `require` 用于指定当前项目所需要的各个依赖以及对应的版本。依赖的版本部分有不同的写法，还有注释，它们并不影响实际的使用，只是作为标识和增强说明的作用。

    - `indirect`注释表示该模块为间接依赖，也就是在当前项目中的`import`语句中并没有发现这个模块的明确引用，有可能是先使用`go get`拉取下来的，也有可能是被项目所依赖的模块依赖的，还有其他的可能。
    - `incompatible`标记标识对应的依赖的版本已经大于 2 了，但是其 module path 中仍然么与添加类似 v2、v3 这样的后缀，不符合 Go Modules 的管理规范，但是并不影响引用。

* `exclude` 用于跳过某个依赖库的版本。一般用于在知道该依赖的某个版本有 bug 或者不兼容的时候，为了安全起见，使用 exclude 命令跳过此版本。

* `replace` 将一个模块版本替换为另一个模块版本，用于解决一些错误的依赖库的引用或者调试依赖库的目的。使用场景有：

    - 在开发中使用第三方库时，可能会做一些定制化的修改。修改之后想引起最小的改动来进行调试，就可以使用 replace 命令将这个依赖重定向为本地的依赖所在路径。不过这个功能在 Go 1.18 开始可以由 work 工作区进行代替了。
    - 有些库不能下载，可以将其 replace 引用到对应的其他仓库位置。比如，可以将`golang.org/x/crypto`库引用到其 GitHub 仓库：`go mod edit -replace golang.org/x/crypto=github.com/golang/crypto@v0.0.0-20160511215533-1f3b11f56072`。

* `retract` 从 Go 1.16 开始引入，用来声明当前项目的某些发行版本不能被其他模块使用，相当于撤回版本，主要目的是将问题更直观的反馈到开发者。

    比如，项目的某些版本中引入了一些严重的问题，可以删除有问题版本的 tag，并在修复后重新打 tag。使用者在发现有问题的版本 tag 丢失后，手动介入升级版本，但是并不清楚缘由。
    
    引入 retract 命令后，维护着可以使用 retract 命令在 go.mod 中添加有问题的版本及对应的提示信息：
    ...
    在重新发布新版本后，引用该依赖库的使用者在执行`go list`命令时就可以看到跳过的版本和“严重bug...”的提醒。
