...
M 的状态不如 P 和 G 那样明确，但是在运行流程的分析中，主要有如下几个状态：

* **运行中**：处于拿到 P 且执行 G 的过程中；
* **运行原生代码**：正在执行原生代码或阻塞的 syscall；
* **休眠中**：M 发现无等待运行的 G 的时候，会进入休眠状态，并加入到空闲列表中；
* **自旋中**：当前工作结束，正在寻找下一个待运行的 G。
